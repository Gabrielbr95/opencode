...
| **What** | Objective, decisions, and current state. | Per Project | `plan/*`, `activeContext.md` |
...
*   **Offline Survivability:** Keep plans small enough to resume after interruptions. 14-day offshore gaps destroy mental context, so we rely on explicit, agent-written, human-triggered state files (`activeContext.md` and `plan/tasks.md`) to pick up exactly where we left off.
...
| **POC** | Answer "can this be done?" | Human-in-loop (Light) | *Optional short in-chat plan* |
| **Script** | Recurring automation. Needs logging. | Human-in-loop | `plan/tasks.md`, `plan/decisions.md` |
| **Application** | Long-lived software. Needs tests/docs. | Human-in-loop (Strict) | `plan/spec.md`, `plan/tasks.md`, `plan/decisions.md` |

*(Note: `activeContext.md` is maintained for Script and Application tiers upon user-triggered session wraps).*
...
The `orchestration` skill is **never** called autonomously by the agent. It is strictly triggered by the user to coordinate autonomous work and optional parallelization across multiple tasks.
...
4.  **Planning:** Generalist writes the required artifacts in `plan/` based on the Tier.
5.  **Approval:** You review and approve the `plan/` files.
6.  **Execution:** Depending on the current mode, Generalist implements directly, loops you in for checks, or you invoke the `orchestration` skill for autonomous execution. Modes can shift fluidly during this step. Tasks are marked sequentially (`[>]` → `[x]` or `[!]`).
7.  **Wrap Session:** You trigger the `/wrap-session` skill. The Generalist writes current state, blockers, and next steps to `activeContext.md`.