---
name: review-plan
description: Model-invoked. The strict review procedure for planning artifacts. Loads when reviewing spec, tasks, decisions, architecture, or overall plan readiness.
---
...
1. **Actionability:** Is the next execution slice clear enough to start safely?
2. **Decision Discipline:** Are major decisions surfaced for user approval? Are minor reversible choices being over-documented?
3. **Task Shape:** Does `plan/tasks.md` show the full path clearly without forcing unnecessary detail everywhere?
4. **YAGNI Check:** Is the plan drifting into unnecessary contract detail or speculative structure that does not materially reduce risk?
...
- A PASS means the plan is coherent enough for the current objective. It does not require every later slice to be fully detailed unless the user explicitly asked for that.
