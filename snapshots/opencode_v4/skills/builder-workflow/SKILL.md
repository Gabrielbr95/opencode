...
description: >-
  ...
  in-progress marking, definition of done, and task report format. Always load
  this alongside the tier-specific skill.
---
...
1. Read `tasks.md`. Identify the `tier:` on the first line and the next unchecked `- [ ]` task. If a `- [-]` task exists, it was interrupted — resume it first.
2. If `spec.md`, `architecture.md`, or `decisions.md` exist, read them for context.
...
- Implementation does not contradict any active decision in `decisions.md`.
- `tasks.md` updated: active task marked `[x]`, any new blockers noted.

## 4. Task Report

After completing a task, update `tasks.md` (`- [-]` to `- [x]`) and output:
