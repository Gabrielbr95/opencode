---
name: gen-decisions
description: Load when reading, writing, or reviewing decisions.md. Defines the decision log format, when to create entries, and how to handle superseded decisions.
---
...
`decisions.md` is the project's persistent engineering memory. LLMs and agents are stateless — every session starts blank. This file ensures that reasoning about architecture, technology choices, and tradeoffs is never lost between sessions or model swaps.

**Rule**: If a design choice is important enough that it might be reconsidered in the future, it must exist in `decisions.md`.

## File Rules

- One file per project: `decisions.md` at the project root
...
### Planner
- Identify decision points during planning and surface them explicitly
- Do not embed architectural choices silently in `spec.md` — record them in `decisions.md`
- Both planner and architect may write new decisions and supersede existing ones

### Architect
- Validate that all architectural choices are recorded before implementation starts
- Explicitly check existing decisions before introducing new ones — avoid duplication
- Both planner and architect may write new decisions and supersede existing ones

### Builder
- Read `decisions.md` before starting implementation
- Do not contradict active decisions
- If implementation requires breaking an active decision: stop, flag the conflict, escalate to planner or architect for resolution before continuing

### Reviewer
- Verify that implementation complies with all active decisions
- Flag any silent overrides or undocumented deviations
- Verify that new decisions were recorded when required
